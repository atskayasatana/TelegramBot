...
def get_latest_launch_foto(url, launch_id):
...
def get_launch_id(url):
    ...
    filter_value=f'start={start_year}-{start_month}-{start_day}&end={end_year}-{end_month}-{end_day}'
...
def get_EPIC(url,params,download_dir_name, number_of_images):
    ttl_imgs = 0
    img_links=[]
    res=[]
    path=Path.cwd()
    ...
    while ttl_imgs<number_of_images:
        url=f'https://api.nasa.gov/EPIC/api/natural/date/{year}-{month}-{day}'
        print(url)
        ...
        new_day=int(day)-1
        day = str(new_day) if new_day>9 else '0'+str(new_day)
    ...
    for link in img_links:
        ...
        url=f'https://api.nasa.gov/EPIC/archive/natural/{year}/{month}/{day}/png/{img_name}.png'
...
if __name__=='__main__':
    path=Path.cwd()
    print(path)
    dir_path=path.joinpath("images")
    Path.mkdir(dir_path, exist_ok=True)
    filename=dir_path.joinpath('hubble.jpeg')
    url='https://upload.wikimedia.org/wikipedia/commons/3/3f/HST-SM4.jpeg'
    response=requests.get('https://api.spacexdata.com/v5/launches/5eb87d42ffd86e000604b384')
    launch_id=get_launch_id('https://api.spacexdata.com/v5/launches','start=2022-01-01&end=2022-08-08')
    spacex_images=get_latest_launch_foto('https://api.spacexdata.com/v5/launches',launch_id)
    print(spacex_images)
    fetch_spacex_last_launch(spacex_images, dir_path)
    print(get_file_extension("https://example.com/txt/hello%20world.txt?v=9#python"))
    query={'api_key':'HkmJlghLQwMT31AhEjDVDRjPBh18Ziep8EuXR7lL',
           'count':50
           }
    get_APOD('https://api.nasa.gov/planetary/apod',query,"APOD")

    params={'api_key':'HkmJlghLQwMT31AhEjDVDRjPBh18Ziep8EuXR7lL'
           }
    print(get_EPIC(url,params,"EPIC", 150))
